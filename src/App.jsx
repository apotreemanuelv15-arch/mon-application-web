...
const apiKey = import.meta.env.VITE_GEMINI_KEY; // Contient votre clé Groq gsk_
...
const translations = {
  fr: { 
    ...
    ai_feedback: "Analyse IA Groq", 
    meditation: "Méditation", 
    placeholder_name: "Votre Nom", 
    placeholder_verse: "Verset", 
    placeholder_text: "Votre révélation...", 
    rank: "Grade",
    live_comm: "COMMUNICATIONS EN DIRECT",
    loading: "COMMUNICATION...",
    mod_title: "COMMANDEMENT DU FRONT",
    launch_live: "LANCER LIVE STUDIO"
  },
  en: { 
    welcome: "HQ Operational", 
    nav_members: "Members", 
    nav_mod: "Moderator", 
    submit: "Submit to HQ", 
    ai_feedback: "Groq AI Analysis", 
    meditation: "Meditation", 
    placeholder_name: "Your Name", 
    placeholder_verse: "Verse", 
    placeholder_text: "Your revelation...", 
    rank: "Rank",
    live_comm: "LIVE COMMUNICATIONS",
    loading: "COMMUNICATING...",
    mod_title: "FRONT COMMAND",
    launch_live: "START LIVE STUDIO"
  },
  pt: { 
    ...
    nav_mod: "Moderador", 
    submit: "Enviar ao QG", 
    ai_feedback: "Análise IA Groq", 
    meditation: "Meditação", 
    placeholder_name: "Seu Nome", 
    placeholder_verse: "Versículo", 
    placeholder_text: "Sua revelação...", 
    rank: "Patente",
    live_comm: "COMUNICAÇÕES AO VIVO",
    loading: "COMUNICANDO...",
    mod_title: "COMANDO DE FRENTE",
    launch_live: "INICIAR LIVE STUDIO"
  }
};
...
const App = () => {
  ...
  const analyzeWithAI = async (e) => {
    ...
    const systemPrompts = {
      fr: "Tu es un mentor spirituel chrétien. Analyse et réponds obligatoirement en FRANÇAIS sous format JSON: { \"encouragement\": \"...\", \"prayer\": \"...\" }.",
      en: "You are a Christian spiritual mentor. Analyze and respond strictly in ENGLISH in JSON format: { \"encouragement\": \"...\", \"prayer\": \"...\" }.",
      pt: "Você é um mentor espiritual cristão. Analise e responda estritamente em PORTUGUÊS no formato JSON: { \"encouragement\": \"...\", \"prayer\": \"...\" }."
    };
    ...
    try {
      const res = await fetch("https://api.groq.com/openai/v1/chat/completions", {
        ...
        body: JSON.stringify({
          ...
          messages: [
            ...
            { role: "user", content: `Verset: ${formData.verse}. Révélation: ${formData.text}` }
          ],
      ...
      await addDoc(collection(db, 'reports'), { ...formData, aiFeedback: cleanJson, timestamp: serverTimestamp(), uid: user.uid, langPreference: lang });
    ...
    <div className="min-h-screen bg-[#050505] text-slate-300">
      <nav className="p-4 border-b border-white/10 flex justify-between items-center bg-black/95 sticky top-0 z-50 backdrop-blur-md">
        ...
        <div className="flex gap-4 items-center">
          ...
          <button onClick={() => setView('member')} className={`text-[10px] font-black uppercase transition-colors ${view === 'member' ? 'text-blue-500' : 'hover:text-white'}`}>{t.nav_members}</button>
          <button onClick={() => setView('mod')} className={`text-[10px] font-black uppercase transition-colors ${view === 'mod' ? 'text-blue-500' : 'hover:text-white'}`}>{t.nav_mod}</button>
        </div>
      </nav>
      ...
      <main className="max-w-7xl mx-auto p-4 md:p-8">
        <div className="grid lg:grid-cols-12 gap-8">
              ...
              <section className="bg-white/5 p-8 rounded-[2rem] border border-white/10 shadow-xl">
                ...
                <form onSubmit={analyzeWithAI} className="space-y-4">
                  <div className="grid md:grid-cols-2 gap-4">
                    <input name="name" placeholder={t.placeholder_name} defaultValue={localStorage.getItem('josue_name') || ""} required className="bg-black border border-white/10 rounded-xl p-5 text-white outline-none focus:border-blue-600 transition-all" />
                    <input name="verse" placeholder={t.placeholder_verse} required className="bg-black border border-white/10 rounded-xl p-5 text-white outline-none focus:border-blue-600 transition-all" />
                  </div>
                  <textarea name="text" placeholder={t.placeholder_text} required rows="3" className="w-full bg-black border border-white/10 rounded-xl p-5 text-white outline-none focus:border-blue-600 transition-all" />
                  <button className="w-full py-5 bg-blue-600 text-white rounded-xl font-black uppercase text-xs tracking-widest shadow-lg shadow-blue-600/20 hover:bg-blue-500 transition-all active:scale-[0.98]">
                    {status === 'loading' ? t.loading : t.submit}
                  </button>
                </form>
                {debugError && <div className="mt-4 p-4 bg-red-900/10 border border-red-500/20 rounded-xl text-red-500 text-[10px] font-mono uppercase italic animate-pulse">Erreur: {debugError}</div>}
              </section>
            )}

            {geminiResult && view === 'member' && (
              <section className="bg-gradient-to-br from-blue-700 to-blue-900 p-8 rounded-[2rem] text-white shadow-2xl animate-in slide-in-from-bottom-4 duration-500">
                ...
                <p className="text-2xl font-bold mb-6 italic leading-tight">"{geminiResult.encouragement}"</p>
            ...
            {view === 'mod' && (
              <section className="space-y-4">
                <div className="bg-white/5 p-6 rounded-[2rem] border border-white/10 flex justify-between items-center shadow-lg">
                  ...
                  <button onClick={() => window.open('https://meet.jit.si/JosueHQ_Live', '_blank')} className="px-6 py-3 bg-red-600 text-white rounded-full font-black text-[10px] animate-pulse flex items-center gap-2 shadow-lg shadow-red-600/20 hover:bg-red-500">
                    <Video size={14}/> {t.launch_live}
                  </button>
                </div>
                {reports.map(r => (
                  <div key={r.id} className="bg-white/5 p-6 rounded-2xl border border-white/5 flex justify-between items-center group hover:border-white/20 transition-all">
          ...
          <aside className="lg:col-span-4 flex flex-col h-[750px] bg-white/5 rounded-[2.5rem] border border-white/10 overflow-hidden shadow-2xl backdrop-blur-sm">
            <div className="p-6 border-b border-white/10 bg-black/40 font-black text-white text-[10px] uppercase tracking-widest flex items-center gap-2"><MessageSquare className="text-blue-500" size={16}/> {t.live_comm}</div>
            ...
            <form onSubmit={sendMsg} className="p-4 bg-black border-t border-white/10 flex gap-2">
              <input value={msgInput} onChange={(e) => setMsgInput(e.target.value)} placeholder="..." className="flex-1 bg-white/5 border border-white/10 rounded-xl px-5 py-4 text-xs text-white outline-none focus:border-blue-600 transition-all" />
              <button type="submit" className="p-4 bg-white text-black rounded-xl hover:bg-blue-600 hover:text-white transition-all active:scale-90"><Send size={20}/></button>
            </form>
          </aside>
        </div>
      </main>
